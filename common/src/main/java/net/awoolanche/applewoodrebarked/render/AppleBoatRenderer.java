...
import net.awoolanche.applewoodrebarked.entities.AppleChestBoatEntity;
...
public class AppleBoatRenderer extends EntityRenderer<Boat>
{
    private final Map<AppleBoatEntity.Type, Pair<ResourceLocation, ListModel<Boat>>> boatResources;

    public AppleBoatRenderer(EntityRendererProvider.Context context, boolean hasChest)
    {
        super(context);
        this.shadowRadius = 0.8F;
        this.boatResources = Stream.of(AppleBoatEntity.Type.values()).collect(ImmutableMap.toImmutableMap((key) -> key, (model) -> {
            return Pair.of(ResourceLocation.fromNamespaceAndPath(AppleWoodRebarked.MOD_ID, getTextureLocation(model, hasChest)), createBoatModel(context, model, hasChest));
        }));
    }
    ...
    {
        if (boat instanceof AppleChestBoatEntity)
            return this.boatResources.get(((AppleChestBoatEntity)boat).getWoodType());
        else
            return this.boatResources.get(((AppleBoatEntity)boat).getWoodType());
    }

    private static String getTextureLocation(AppleBoatEntity.Type model, boolean hasChest)
    {
    ...
    {
        return new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(AppleWoodRebarked.MOD_ID, name), layer);
    }

    public static ModelLayerLocation createBoatModelName(AppleBoatEntity.Type model)
    {
        return createLocation("boat/" + model.getName(), "main");
    }

    public static ModelLayerLocation createChestBoatModelName(AppleBoatEntity.Type model)
    {
        return createLocation("chest_boat/" + model.getName(), "main");
    }

    private BoatModel createBoatModel(EntityRendererProvider.Context context, AppleBoatEntity.Type model, boolean hasChest)
    {
